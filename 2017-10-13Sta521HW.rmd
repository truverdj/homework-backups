...
for (i in 1:nsim){
  ...
  rmseList.nb[i] = rmse(College.train$Apps, ypred_i)
}
...
Well, that comparison to the out of sample (test) RMSE is very dissapointing. The model seems to fit the observed data well but does not do was well with new data not used in the construction of the model. This last model, the negative binomial, does seem the best by all other diagnostics and the disparity between training RMSE and test RMSE is smaller than the other models we have constructed. We note that the RMSE values for all of these models are very poor, even considering the scale of the variable we are trying to predict, Apps, which has a range of 81 to 20192 and 75% of its values between 512 and 6642.  
...
glmCoverage = function(model, newdata, nsim){ # hold onto your butts; it's about to get empirical 
  tempMatrix = matrix(NA, ncol = nsim, nrow = dim(newdata)[1])
  set.seed(1)
  class(model) = "glm"
  modelMatrix = model.matrix(model, data = newdata)
  simMatrix = sim(model, nsim)
  for (j in 1:nsim){
    y_pred = modelMatrix %*% simMatrix@coef[j,]
    tempMatrix[,j] = y_pred
  }
  intervalMatrix = matrix(NA, ncol = 2, nrow = nsim)
  for (i in 1:nsim){
...
normal2.coverage = coverage(normal2.pred, College.test$Apps)
# https://stackoverflow.com/questions/14423325/confidence-intervals-for-predictions-from-logistic-regression
poisson.pred = predict(apps.glm7, newdata = College.test, type = "response" )
```
